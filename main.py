...
# Show training data for debugging
st.write('✅ Training Data Used:', train.tail())

# ARIMA modeling

from pmdarima.arima import auto_arima

# Use auto_arima to find the best (p,d,q) and fit the model
model = auto_arima(train['Total_HIV_AIDS_Deaths'], seasonal=False, stepwise=True, suppress_warnings=True, trace=True)

try:
    forecast = model.predict(n_periods=forecast_steps)
    st.write("🧪 Forecasted Values (auto_arima):", forecast)
except Exception as e:
    st.error(f"Auto ARIMA failed: {e}")
    from statsmodels.tsa.arima.model import ARIMA
    fallback_model = ARIMA(train['Total_HIV_AIDS_Deaths'], order=(3,2,2)).fit()
    forecast = fallback_model.forecast(steps=forecast_steps)
    st.write("🛠 Fallback Forecast (ARIMA 3,2,2):", forecast)
