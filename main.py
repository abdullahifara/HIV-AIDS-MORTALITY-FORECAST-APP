...
st.markdown("This app forecasts future HIV/AIDS deaths using ARIMA(3,2,2) to match notebook results.")
...
# Forecasting setup
train = data  # Use full data to match notebook
forecast_steps = 7  # 2024 to 2030
start_year = 2024
forecast_index = pd.date_range(start=f"{start_year}-12-31", periods=forecast_steps, freq="Y")

# Fit ARIMA(3,2,2) model
...
with tab3:
    st.subheader("ARIMA Forecast (2024–2030)")
    ...
    ax3.set_title("Forecast from 2024 to 2030 using ARIMA(3,2,2)")
    ax3.legend()
    st.pyplot(fig3)
...
with tab5:
    ...
    st.download_button("Download Forecast CSV", data=csv, file_name='hiv_forecast_2024_2030.csv', mime='text/csv')
