...
with tab4:
    ...
    with res_tabs[0]:
        ...
        rmse_train = mean_squared_error(aligned_train_actual.values, model_fit.fittedvalues.values, squared=False)
        mae_train = mean_absolute_error(aligned_train_actual.values, model_fit.fittedvalues.values)

        st.markdown("**Training Error Metrics**")
        st.write(f"**RMSE:** {rmse_train:,.0f}")
        st.write(f"**MAE:** {mae_train:,.0f}")
    ...
    with res_tabs[1]:
        ...
        rmse_test = mean_squared_error(test['Total_HIV_AIDS_Deaths'].values, test_forecast.values, squared=False)
        mae_test = mean_absolute_error(test['Total_HIV_AIDS_Deaths'].values, test_forecast.values)

        st.markdown("**Testing Error Metrics**")
        st.write(f"**RMSE:** {rmse_test:,.0f}")
        st.write(f"**MAE:** {mae_test:,.0f}")
with tab5:
