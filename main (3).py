...
# Sidebar forecast range
forecast_steps = st.sidebar.slider("Forecast years into the future", min_value=1, max_value=10, value=7)
start_year = data.index[-1].year + 1
...
# Tabs
with st.tabs(["Overview", "EDA", "Forecast", "Residuals", "Download"]):
    with st.container():
    ...
    with st.container():
        ...
        ax2.plot(forecast_df, label="Forecast", linestyle="--")
        ax2.set_title("Forecast ({}–{})".format(start_year, future_years[-1].year))
        ax2.legend()
        st.pyplot(fig2)
