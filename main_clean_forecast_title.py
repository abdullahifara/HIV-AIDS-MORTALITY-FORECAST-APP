...
with tab4:
    st.subheader("Residual Analysis: Training and Testing Residuals")
    ...
    test_residuals.index = test.index  # ensure matching index

    # Combine both for display
    full_residuals = pd.concat([train_residuals, test_residuals])

    fig, ax = plt.subplots(figsize=(12, 4))
    ax.plot(train_residuals.index, train_residuals.values, label='Training Residuals', color='blue')
    ax.plot(test_residuals.index, test_residuals.values, label='Testing Residuals', color='purple')
    ax.axhline(0, linestyle='--', color='red')
    ax.set_title("Residuals Over Time (1990–2023)")
    ax.set_xlabel("Year")
    ...
    rmse_train = mean_squared_error(train['Total_HIV_AIDS_Deaths'], model_fit.fittedvalues, squared=False)
    mae_train = mean_absolute_error(train['Total_HIV_AIDS_Deaths'], model_fit.fittedvalues)

    rmse_test = mean_squared_error(test['Total_HIV_AIDS_Deaths'], test_forecast, squared=False)
    mae_test = mean_absolute_error(test['Total_HIV_AIDS_Deaths'], test_forecast)

    st.markdown("### Error Metrics:")
    st.write(f"**Training RMSE:** {rmse_train:,.0f}")
    st.write(f"**Training MAE:** {mae_train:,.0f}")
    st.write(f"**Testing RMSE:** {rmse_test:,.0f}")
    st.write(f"**Testing MAE:** {mae_test:,.0f}")
with tab5:
