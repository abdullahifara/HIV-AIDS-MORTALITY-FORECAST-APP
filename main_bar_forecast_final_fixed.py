...
with tab2:
    ...
    ax2.bar(data.index.year, data["Total_HIV_AIDS_Deaths"], color='skyblue')
...
with tab3:
    ...
    forecast_years_str = [str(date.year) for date in forecast_df.index]
    forecast_values = forecast_df["Predicted_HIV_Deaths"].values
    ...
    ax3.bar(forecast_years_str, forecast_values, label="Forecast (Bar)", color='green')
